...
//   to fetch the doctors from the server
  useEffect(() => {
    let mounted = true;
    async function load() {
      setLoading(true);
...
// when we are trying to refetch the 
  const retry = async () => {
    setLoading(true);
    setError("");
    try {
                ...
                <div className={d.wrapper}>
                    ...
                    <div className={d.searchContainer}>
                        <div className={d.searchWrapper}>
                            <input type="text" placeholder="Search Doctor by Name or Specialization.." value={searchTerm}
                                onChange={(e)=>setSearchTerm(e.target.value)}
                                className={d.searchInput}
                            />
                            <Search className={d.searchIcon}/>
                            {searchTerm.length > 0 && (
                                <button onClick={()=>setSearchTerm("")} className={d.clearButton}>
