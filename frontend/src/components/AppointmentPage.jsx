...
import { appointmentPageStyles as a,cardStyles as c,badgeStyles as b,iconSize } from '../assets/dummyStyles'
...
const AppointmentPage = () => {
  ...
  return (
    <div className={a.pageContainer}>
        ...
        <div className={a.maxWidthContainer}>
            <h1 className={a.doctorTitle}>
                Your Doctor Appointments
            </h1>
            {loadingDoctors &&(
                <div className={a.loadingText}>
                    Loading Doctors...
                </div>
            )}
            {!loadingDoctors && appointmentData.length === 0 && (
                <div className={a.emptyStateText}>
                    No doctor appointments found!
                </div>
            )}
            <div className={a.doctorGrid}>
                {appointmentData.map((item)=>(
                    <div key={item.id} className={c.doctorCard}>
                       <div className={c.doctorImageContainer}>
                             <img src={item.image || "/placeholder-doctor.png"} alt={item.doctor}
                            className={c.image} loading="lazy"
                        />
                       </div>
                       <h2 className={c.doctorName}>
                            {item.doctor}
                       </h2>
                       <div className={c.specialization}>
                            {item.specialization}{" "} {item.experience ? `${item.experience} `: ""}
                       </div>
                       <p className={c.dateContainer}>
                        <CalendarDays className={iconSize.medium}/> {item.date}
                       </p>
                       <p className={c.timeContainer}>
                        <Clock className={iconSize.medium}/> {item.time}
                       </p>
                       <div className={c.badgesContainer}>
                        <PaymentBadge payment={item.payment}/>
                        <StatusBadge itemStatus={item.status}/>
                       </div>
                       {item.status === "Rescheduled" && item.rescheduledTo ? (
                            <div className={c.rescheduledText}>
                                Reschedule to {" "}
                                <span className={c.rescheduledSpan}>
                 ...
                 <h1 className={a.serviceTitle}>
                Your Booked Services
            </h1>
            {loadingServices &&(
                <div className={a.serviceLoadingText}>
                    Loading Service Booking...
                </div>
            )}
            {!loadingServices && serviceData.length === 0 && (
                <div className={a.serviceEmptyStateText}>
                    No services bookings found!
                </div>
            )}
            <div className={a.serviceGrid}>
          {serviceData.map((srv) => (
            <div key={srv.id} className={c.serviceCard}>
              <div className={c.serviceImageContainer}>
                <img
              ...
              <h3 className={c.serviceName}>{srv.name}</h3>

              <p className={c.price}>₹{srv.price}</p>

              <p className={c.serviceDateContainer}>
                <CalendarDays className={iconSize.medium} /> {srv.date}
              </p>

              <p className={c.serviceTimeContainer}>
              ...
              {srv.status === "Rescheduled" && srv.rescheduledTo ? (
                <div className={c.serviceRescheduledText}>
                  Rescheduled to{" "}
                  <span className={c.rescheduledSpan}>
