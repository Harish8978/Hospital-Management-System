  ...
  return (
    <div className={l.pageContainer}>
        <div className={l.contentWrapper}>
            <div className={l.headerContainer}>
                <div>
                    <h1 className={l.headerTitle}>
                        All Appointments
                    </h1>
                ...
                <div className={l.searchFilterContainer}>
                    <div className={l.searchContainer}>
                        <div className={l.searchIconContainer}>
                            <Search className={l.searchIcon}/>
                        </div>
                        <input value={search} onChange={(e)=>setSearch(e.target.value)}
                         className={l.searchInput} placeholder="Search Patient Name"   
                        />
                        {search && (
                            <button onClick={()=>setSearch("")} className={l.clearSearchButton}>
                                <X className={l.clearSearchIcon}/>
                            </button>
                        )}
                    </div>
                    <select
            ...
            {loading ? (
                <div className={l.loadingContainer}>
                    Loading Appointments...
                </div>
            ): error?(
                <div className={l.errorContainer}>Error:{error}</div>
            ):(
                <div className={l.appointmentsGrid}>
                    {filtered.map((a)=>(
                        <article key={a.id} className={l.appointmentCard}>
                            <header className={l.cardHeader}>
                                <div className={l.cardAvatar}>
                                    {a.doctor.image?(
                                        <img src={a.doctor.image} alt={a.doctorName} className={l.cardAvatarImage}
                                            onError={(e)=>{e.currentTarget.style.display="none"}}
                                        />
                                    ):(
                                        <div className={l.cardAvatarFallback}>
                                            {
                                ...
                                <div className={l.cardContent}>
                                    <div className={l.cardPatientName}>
                                        {a.patient}
                                    </div>
                                    <div className={l.cardPatientInfo}>
                                        {a.age} yrs &middot; {a.gender}
                                    </div>
                                    <div className={l.cardDoctorInfo}>
                                        <span className={l.cardDoctorName}>
                                            {a.doctorName}
                                        </span>
                                    </div>
                                    <div className={l.cardSpeciality}>
                                        {a.speciality}
                                    </div>
                                </div>
                            </header>
                                    
                            <div className={l.dateTimeSection}>
                                <div className={l.dateTimeContainer}>
                                    <Calendar className={l.calendarIcon} />
                                ...
                                <div className={l.feeText}>₹{a.fee}</div>
                            </div>
                            <div className={l.contactStatusSection}>
                                <div className={l.phoneContainer}>
                                    <Phone className={l.phoneIcon}/>
                                    <span className={l.phoneNumber}>{a.mobile}</span>
                                </div>
                                <div className={l.statusContainer}>
                                    <StatusBadge status={a.status}/>
                                    <StatusSelect appointment={a} onChange={(s)=>updateStatus(a.id,s)}/>
                                </div>
                            </div>
                            <div className={l.rescheduleContainer}>
                                <RescheduleButton
                                    appointment={a}
