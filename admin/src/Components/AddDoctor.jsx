...
import { Calendar, CheckCircle, Eye, EyeClosed, Plus, Trash2, User, XCircle } from 'lucide-react'; // Ensure you import your icons!
import { doctorDetailStyles as s } from "../assets/dummyStyles";

// HELPER FUNCTIONS (Fine to keep outside the component as pure functions)
...
const AddDoctor = () => {
  // Move all states and hooks INSIDE the component
  ...
  // Shows toast and removes it in 3000ms
  useEffect(() => {
    ...
    <div className={s.pageContainer}>
      ...
      <div className={`${s.maxWidthContainer} ${s.formContainer}`}>
        <form onSubmit={handleAdd} className={s.formGrid}>
          ...
          <input className={s.inputBase} placeholder="Experience (e.g., 10 years)" value={form.experience} onChange={(e)=>setForm({...form,experience:e.target.value})} />
          ...
          <input className={s.inputBase} placeholder="Doctor Email" value={form.email} onChange={(e)=>setForm({...form,email:e.target.value})} type="email" />
        ...
        <div className={s.submitButtonContainer}>
            <button type="submit" disabled={loading} className={s.submitButton + " " + s.cursorPointer + " " + (loading ? s.submitButtonDisabled : s.submitButtonEnabled)}>
                {loading? "Adding..." : "Add Doctor to the Team"}
            </button>
        </div>
        </form>
      </div>
