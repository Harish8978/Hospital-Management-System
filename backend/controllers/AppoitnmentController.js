...
// Helper functions
// this function will provide you the finite number
const safeNumber = (v) => {
  const n = Number(v);
  return Number.isFinite(n) ? n : null;
};

// this function will provide you the frontend url
    ...
    try {
        ...
        if(!resolvedCreatedBy || !req.query.mobile){
             return res.status(400).json({
                success:false,
                message:"Authentication required or mobile number missing"
             })
        }
        const filter = {};
        if(resolvedCreatedBy) filter.createdBy = resolvedCreatedBy;
        if(req.query.mobile) filter.mobile = req.query.mobile;
        const items = await Appointment.find(filter).sort({date:1,time:1}).lean();
        return res.json({
            success:true,
            appointments:items,
        })
         ...
         console.error("Get Appointments By Patient Error",error.message || error);
        return res.status(500).json({
            success:false,
            message:"Server Error"
        })
...
// to create an appointment
    ...
    try {
        ...
        const numericFees = safeNumber(fees ?? fee ?? 0);
        if (numericFees === null || numericFees < 0) {
            return res.status(400).json({
                success:false,
                message:"Invalid fees provided"
            })
        }
        ...
        if (existingAppointment) {
            return res.status(409).json({
                success:false,
                message:"You already have an appointment with this doctor at the same date and time."
            })
        }
        let doctor = null;
        try {
...
//  to confrim the  online payment and make it paid
export const confirmPayment  = async(req,res)=>{
    try {
        ...
        try {
            ...
            console.error("Stripe retrieve session error:", error.message || error);
            return res.status(404).json({
                success:false,
    ...
    return res.json({ success: true, appointment: appt });
        ...
        console.error("ConfirmAppointmentPaymentError",error.message || error);
        return res.status(500).json({
            success:false,
            message:"Server Error"
        })
...
export const updateAppointment = async(req,res)=>{
    try {
        ...
        console.error("UpdateAppointmentError",error.message || error);
        return res.status(500).json({
            success:false,
            message:"Server Error"
        })   
...
export const cancelAppointment = async(req,res)=>{
    try {
        ...
        console.error("Cancel AppointmentError",error.message || error);
        return res.status(500).json({
            success:false,
            message:"Server Error"
        })
...
export const getStats = async(req,res)=>{
    try {
        ...
        console.error("GetStatsError",error.message || error);
        return res.status(500).json({
            success:false,
            message:"Server Error"
        })
...
// to get the registered user count
export async function getRegisteredUserCount(req, res){
    try {
        const totalUsers = await clerkClient.users.count();
        return res.json({
            success:true,
            totalUsers
        })
    } catch (error) {
        console.error("getCountOfRegisteredUsersError",error.message || error);
        return res.status(500).json({
            success:false,
            message:"Server Error"
        })
