...
// to confrim the service payment
...
export const getServiceAppointments = async(req,res)=>{
  try {
       const { serviceId, mobile, status, page: pageRaw = 1, limit: limitRaw = 50, search = "" } = req.query;
        ...
        return res.json({ success: true, appointments, meta:{total, page, limit,count:appointments.length} });
  } catch (error) {
      console.error("getServiceAppointments error:", error);
       return res.status(500).json({ success: false, message: "Server error" });
  }
}
// to get serviceappointments by id
export const getServiceAppointmentById = async(req,res)=>{
  try {
    ...
    if(!appointment) return res.status(404).json({message:"Service appointment not found"})
...
export const updateServiceAppointment = async(req,res)=>{
  try {
    ...
    const body = req.body || {};
    const updates  = {};
    // first check whether it is filled if yes then update the field
    ...
    const updated = await ServiceAppointment.findByIdAndUpdate(id,{$set:updates},{new:true,runValidators:true}).lean();
    if(!updated) return res.status(404).json({message:"Service appointment not found after update"});
...
// to cancel a serviceAppointment
export const cancelServiceAppointment = async(req,res)=>{
  try {
    ...
    await appt.save();
    return res.json({ success: true, message: "Appointment canceled", appointment: appt });
  } catch (error) {
...
export const getServiceAppointmentStats = async(req,res)=>{
  try {
    const services = await Service.aggregate([
      ...
      { $sort: { createdAt: -1 } },
    ]) // it will give you total appointment , complete appt , cancel appt and total earning done by them
    console.log(services)
    return res.json({ success: true, stats: services,totalServices:services.length });
