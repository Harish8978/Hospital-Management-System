...
// Remove the duplicate slots and sort by time
...
// Convert JSON data into plain structure for client
...
export async function createDoctor(req, res) {
  try {
    ...
    if (!secret) {
      console.warn("Your JWT Secret is not defined");
    ...
    const token = jwt.sign(
      { id: doc.id.toString(), email: doc.email, role: "doctor" },
      secret,
      { expiresIn: "7days" }
    );
  ...
  try {
    ...
    if (existing.imagePublicId) {
      try {
        ...
        console.warn("DeleteFromCloudinary Error:", error.message || error);
      }
    }
    ...
    console.error("DeleteDoctorError:", error.message || error);
    return res.status(500).json({
      success: false,
      message: "Server Error"
    });
...
export async function toggleAvailability(req, res) {
  try {
    ...
    if (!doc) return res.status(404).json({ success: false, message: "Cannot find Doctor" });
    ...
    console.error("ToggleAvailabilityError:", error.message || error);
    return res.status(500).json({
      success: false,
      message: "Server Error"
    });
...
export async function doctorLogin(req, res) {
  try {
    ...
    console.error("Login Doctor Error:", error.message || error);
    return res.status(500).json({
      success: false,
      message: "Server Error"
    });
